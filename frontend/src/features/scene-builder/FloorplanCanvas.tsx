  ...
  function move(event: React.PointerEvent<SVGSVGElement>) {
    ...
    onChange({
      ...
      objects: {
        ...
        [drag.id]: {
          ...item,
          grid_x: Math.max(0, Math.min(room.width_cells - item.width_cells, Math.round(cursor.x - room.grid_x - drag.offsetX))),
          grid_y: Math.max(0, Math.min(room.depth_cells - item.depth_cells, Math.round(cursor.y - room.grid_y - drag.offsetY))),
        },
      },
    });
  }
