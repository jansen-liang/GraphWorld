...
# label | formula | clean evidence | EFE mode | selector | outcome learning
CONDITIONS=(
  "01_global_ab|global_ab|none|generative|efe_all|1"
  "02_rule_reference|global_ab|none|generative|rule_all|0"
  "03_stale_no_clean|stale_h005|none|goal_conditioned|efe_all|1"
  "04_stale_clean_w025|stale_h005|clean_w025|goal_conditioned|efe_all|1"
  "05_stale_clean_w050|stale_h005|clean_w050|goal_conditioned|efe_all|1"
  "06_stale_clean_w100|stale_h005|clean_w100|goal_conditioned|efe_all|1"
)
...
echo "============================================================"
echo "EFE clean-entry evidence long experiment v1"
...
for seed in "${SEEDS[@]}"; do
  for spec in "${CONDITIONS[@]}"; do
    IFS='|' read -r label formula clean_variant efe_mode authority learning <<< "$spec"
    group=$((group + 1))
    GROUP_START=$SECONDS
    if (( group == 1 )); then
