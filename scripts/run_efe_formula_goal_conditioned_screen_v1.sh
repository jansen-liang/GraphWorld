...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/backend/data/experiments/efe_formula_goal_conditioned_screen_v1}"
EXPECTED_SECONDS_PER_GROUP="${EXPECTED_SECONDS_PER_GROUP:-120}"
NO_LLM="${NO_LLM:-0}"

# label | scoring formula | selector | online goal-family posterior update
CONDITIONS=(
...
echo "============================================================"
echo "EFE formula experiment: goal-conditioned Bayesian screen v1"
echo "实验目的: 区分公式结构收益与在线后验学习收益"
echo "总实验组: $TOTAL_GROUPS"
echo "场景: simple_hospital_1f | seed=$SEED | 每组=$STEPS steps"
...
for spec in "${CONDITIONS[@]}"; do
  ...
  echo "名称: $label"
  echo "公式: $efe_mode | 选择器: $authority | 在线学习: $learning"
  echo "开始前预计剩余: 约 $((REMAINING_SECONDS / 60)) 分钟"
...
echo
echo "============================================================"
echo "全部 $TOTAL_GROUPS/$TOTAL_GROUPS 组完成"
