...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/backend/data/experiments/efe_goal_policy_v1_quick}"
NO_LLM="${NO_LLM:-0}"
...
# label|EFE_MODE|authority|goal-outcome-learning
CONDITIONS=(
  "generative_efe|generative|efe_all|1"
  "goal_conditioned_learned|goal_conditioned|efe_all|1"
  "goal_conditioned_frozen|goal_conditioned|efe_all|0"
  "rule_all|goal_conditioned|rule_all|0"
)

echo "EFE goal-policy v1 quick experiment"
echo "  total groups: 4"
...
for spec in "${CONDITIONS[@]}"; do
  ...
  echo "===== group $group/4 | $label | mode=$efe_mode | authority=$authority ====="
