...
MODES=("single_round" "efe_learned")
TOTAL_GROUPS=$((${#SCENES[@]} * ${#MODES[@]}))

mkdir -p "$OUTPUT_ROOT/logs"

echo "Mainline EFE versus single_round: 5 scenes x 2 modes"
...
for scene_spec in "${SCENES[@]}"; do
  ...
  for mode in "${MODES[@]}"; do
    group=$((group + 1))
    log_path="$OUTPUT_ROOT/logs/${scene}__${mode}__seed_${SEED}.log"
    condition_root="$OUTPUT_ROOT/$mode/seed_$SEED"
    echo "===== group $group/$TOTAL_GROUPS | scene=$scene | mode=$mode | humans=$humans ====="

    if compgen -G "$condition_root/$scene/*/*/summary.json" > /dev/null; then
      echo "SKIP: completed summary already exists for $scene / $mode / seed $SEED"
      continue
    fi
